...
- Windows 11 x64: [v2.0.0 release assets](https://github.com/charlotterunrun-bot/RollCall/releases/tag/v2.0.0), asset name `RollCall-2.0.0-Windows-x64.exe`.
- macOS 15+ Apple Silicon: [v2.0.0 release assets](https://github.com/charlotterunrun-bot/RollCall/releases/tag/v2.0.0), asset name `RollCall-2.0.0-macOS-arm64.zip`.
...
The source is in `src/`, with `src/version.py` as the single version source. The application version is `2.0.0`; the Windows file metadata is `2.0.0.0`, while the product and package version strings are `2.0.0`. Build locally with Python 3.12 and the pinned requirements:
...
The platform scripts produce `RollCall-2.0.0-Windows-x64.exe` and `RollCall-2.0.0-macOS-arm64.zip`. Do not put real rosters, attendance, credentials, virtual environments, or temporary evidence in a public repository.
